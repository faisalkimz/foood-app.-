...
export default function Orders({ searchQuery }) {
  ...
  const handleAdvanceStatus = (id) => {
    setOrders(prev => prev.map(o => {
      ...
      if (idx >= 0 && idx < STATUS_FLOW.length - 1) return { ...o, status: STATUS_FLOW[idx + 1] };
      return o;
    }));
  };
          ...
          return (
            <div className="stat-card" key={key} style={{ cursor: 'pointer' }} onClick={() => setFilter(filter === key ? 'all' : key)}>
              ...
              <div className="stat-info">
                <div className="stat-value" style={{ fontSize: 20 }}>{counts[key]}</div>
      ...
      <div className="filter-bar">
        {['all', ...Object.keys(STATUS_LABELS)].map((f) => (
          <button key={f} className={`filter-pill ${filter === f ? 'active' : ''}`} onClick={() => setFilter(f)}>
            {f === 'all' ? `All (${counts.all})` : `${STATUS_LABELS[f]} (${counts[f]})`}
          </button>
        ))}
      </div>
              ...
              <tr key={o.id}>
                ...
                <td>
                  <div className="table-actions">
                    ...
                    {STATUS_FLOW.includes(o.status) && o.status !== 'delivered' && (
                      <button className="btn btn-sm btn-success" onClick={() => handleAdvanceStatus(o.id)}>→</button>
                    )}
                    {o.status !== 'cancelled' && o.status !== 'delivered' && (
                      <button className="btn btn-sm btn-danger" onClick={() => handleCancelOrder(o.id)}>✕</button>
                    )}
                  </div>
                </td>
              </tr>
        ...
        <div className="modal-overlay" onClick={() => setSelectedOrder(null)}>
          <div className="modal" style={{ width: 540 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="order-detail-grid">
                ...
                <div className="detail-item">
                  ...
                  <div className="detail-value">{selectedOrder.deliveryTime}</div>
                </div>
                <div className="detail-item">
                ...
                <div className="detail-item">
                  ...
                  <div className="detail-value" style={{ color: 'var(--primary)', fontSize: 16 }}>${selectedOrder.total.toFixed(2)}</div>
                </div>
              </div>

              <div className="detail-label" style={{ marginBottom: 8 }}>Order Items</div>
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  {STATUS_FLOW.map((step, idx) => {
                    const ci = STATUS_FLOW.indexOf(selectedOrder.status);
                    const done = idx <= ci;
            ...
            <div className="modal-footer">
              {STATUS_FLOW.includes(selectedOrder.status) && selectedOrder.status !== 'delivered' && (
                <>
                  <button className="btn btn-danger" onClick={() => handleCancelOrder(selectedOrder.id)}>Cancel</button>
                  <button className="btn btn-primary" onClick={() => {
                    handleAdvanceStatus(selectedOrder.id);
                    const ni = STATUS_FLOW.indexOf(selectedOrder.status) + 1;
                    if (ni < STATUS_FLOW.length) setSelectedOrder(prev => ({ ...prev, status: STATUS_FLOW[ni] }));
                  }}>
                    Advance →
                  </button>
                </>
